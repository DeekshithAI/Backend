...
from db import crud
from topview.model import TopViewModel, assign_numbers, draw_overlay
...
topview_model = TopViewModel("topview/final_best.pt")
...
async def upload_topview(
    ...
    # Detect trees
    boxes = topview_model.detect_trees(img)
    
    # Convert to centroids and assign numbers
    centroids = [{"cx": b["cx"], "cy": b["cy"], "conf": b.get("conf", 1.0)} for b in boxes]
    ...
    draw_overlay(img, numbered, out_path=annotated_path, pin_radius=60)
    ...
    if survey.topview_image_path and os.path.exists(survey.topview_image_path):
        ...
        if os.path.exists(original_path):
            ...
            if img is not None:
                ...
                draw_overlay(img, centroids, out_path=survey.topview_image_path, pin_radius=60)
